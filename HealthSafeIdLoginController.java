...
public class HealthSafeIdLoginController extends ScopeBasedController {
	...
	public Map<String, String> lookupUserId(@RequestBody Map<String, String> filter,
			...
			@RequestParam(value = AppConstants.OPTUMID_HEADER_LANGUAGE, required = false) String lang) {
		{
			...
			try {
				...
				Object searchwithAllCerteriaObject = findSharedElement(filter, false);
				Object searchWithEmailOnlyObject = findSharedElement(filter, true);

				if (searchwithAllCerteriaObject instanceof Resources
						&& searchWithEmailOnlyObject instanceof Resources) {
					Resources searchwithAllCerteria = (Resources) searchwithAllCerteriaObject;
					Resources searchWithEmailOnly = (Resources) searchWithEmailOnlyObject;

					if (searchwithAllCerteria == null || ((Resources) searchwithAllCerteria).getResource() == null
							|| searchWithEmailOnly == null || ((Resources) searchWithEmailOnly).getResource() == null) {
					...
					logger.info("search Email Count " + searchEmailCount);
					logger.info("search All Criteria Count " + searchAllCriteriaCount);
					...
					logger.info("Email Response" + emailResp);
					if ((emailResp instanceof String) && StringUtils.isNotBlank(emailResp.toString())) {
	...
	private String setupDefault(String targetURL) throws URISyntaxException {
		...
		if (StringUtils.startsWithIgnoreCase(targetPortalURL, "http")) {
			...
			if (!StringUtils.contains(targetURL, AppConstants.OPTUMID_HEADER_ELIGIBILITY) && portalSetting != null
					&& portalSetting.getPortalEligibility() != null) {
				targetURL += "&" + AppConstants.OPTUMID_HEADER_ELIGIBILITY + "=" + portalSetting.getPortalEligibility();
			}
